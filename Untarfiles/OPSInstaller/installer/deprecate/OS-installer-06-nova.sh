...
#
# placement 
ssh -t openstack@controller sudo /bin/bash -x ./OPSInstaller/controller/exe-stage15-x1-SUDO-placement-database.sh | tee log/s15-x1-controller.log
ssh -t openstack@controller /bin/bash -x ./OPSInstaller/controller/exe-stage15-x2-USER-placement-endpoints.sh | tee log/s15-x2-controller.log
ssh -t openstack@controller sudo /bin/bash -x ./OPSInstaller/controller/exe-stage15-x3-SUDO-placement.sh | tee log/s15-x3-controller.log
#
# nova
#
...
ssh -t openstack@compute sudo /bin/bash -x ./OPSInstaller/compute/exe-stage19-x1-SUDO-nova-compute.sh | tee log/s19-x1-compute.log
sleep 5
if [ "$intype" == "$full" ]
then
ssh -t openstack@compute1 sudo /bin/bash -x ./OPSInstaller/compute1/exe-stage19-SUDO-nova-compute.sh | tee log/s19-compute.log
ssh -t openstack@compute1 sudo /bin/bash -x ./OPSInstaller/compute1/exe-stage19-x1-SUDO-nova-compute.sh | tee log/s19-x1-compute.log
fi
sleep 5
